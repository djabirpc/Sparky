...
namespace Bongo.Core
{
    ...
    class StudyRoomBookingServiceTests
    {
        ...
        [TestCase(0, false)]
        [TestCase(55, true)]
        public void StudyRoomBooking_BookRoomWithAvailibility_ReturnsBookingId
            (int expectedBookingId, bool roomAvaibility)
        {
            if (!roomAvaibility)
            {
                _availableStudyRoom.Clear();
        ...
        [Test]
        public void BookNotInvoked_SaveBookingWithoutAvailableRoom_BookMethodNotInvoke()
        {
            _availableStudyRoom.Clear();
            var result = _bookingService.BookStudyRoom(_request);
